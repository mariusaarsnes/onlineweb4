        ...
        if form.is_valid():
            ...
            if request.user.started_date and request.user.field_of_study:
                ...
                if request.user.started_date == started_date and request.user.field_of_study == field_of_study:
                    messages.error(
                        request, _("Du er allerede registrert med denne studieretningen og denne startdatoen.")
                    )
                    return redirect("profiles_active", active_tab="membership")
...
def get_expiry_date(started_year, length_of_fos):
    ...
    # lists from NTNU
    new_expiry_date = datetime.date(started_year, 9, 16) + datetime.timedelta(days=365 * length_of_fos)
    ...
    if new_expiry_date < today:
        if today < datetime.date(today.year, 9, 15):
            ...
            new_expiry_date = datetime.date(today.year, 9, 16) + datetime.timedelta(days=365)
...
def create_membership_application(request):
    if request.method == "POST":
        ...
        application = MembershipApproval(
            applicant=request.user, field_of_study=request.user.field_of_study, new_expiry_date=new_expiration_date
        )
        application.save()
