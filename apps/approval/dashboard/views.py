...
def index(request):
    ...
    context["processed_applications"] = MembershipApproval.objects.filter(processed=True).order_by("-processed_date")[
        :10
    ]
...
Om feilen vedvarer etter en refresh, kontakt dotkom@online.ntnu.no."""
            ...
            if not user.ntnu_username:
                response_text = json.dumps(
                    {"message": _("""Brukeren (%s) har ikke noe lagret ntnu brukernavn.""") % user.get_full_name()}
                )
                return HttpResponse(status=412, content=response_text)
