    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if order:
                ...
                # Creating new order
                order = Order(product=product, price=product.price, quantity=quantity, size=size, order_line=order_line)
            order.save()
    ...
    def get(self, request, *args, **kwargs):
        ...
        if order_line:
            invalid_orders = order_line.orders.filter(
                Q(product__active=False) | Q(product__deadline__lt=timezone.now()) | Q(product__stock=0)
            )
