...
class ProductReadOnlySerializer(serializers.ModelSerializer):
    ...
    related_products = serializers.PrimaryKeyRelatedField(many=True, queryset=Product.objects.filter(active=True))
...
class OrderCreateSerializer(serializers.ModelSerializer):
    ...
    size = serializers.PrimaryKeyRelatedField(default=None, queryset=ProductSize.objects.all())
    ...
    def create(self, validated_data):
        ...
        order = super().create(
            {**validated_data, "price": product.price}  # Add the current price of the product as the price of the order
        )
...
class OrderUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate_size(self, size: ProductSize):
        ...
        if allowed_sizes.count() > 0 and size not in allowed_sizes:
            raise serializers.ValidationError(f"Denne størrelsen er ikke tilgjengelig for dette produktet")
...
class OrderLineReadOnlySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = OrderLine
        fields = ("id", "datetime", "paid", "stripe_id", "delivered", "orders", "subtotal", "is_valid", "payment")
        read_only = True
...
class OrderLineCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = OrderLine
        fields = ("id", "datetime", "paid", "stripe_id", "delivered", "orders", "subtotal", "is_valid", "user")
        read_only_fields = ("id", "datetime", "paid", "stripe_id", "delivered", "subtotal", "is_valid")
