...
class WebshopOrderLineTests(OIDCTestCase):
    def setUp(self):
        ...
        self.product1: Product = G(Product, name="Onlinegenser", deadline=timezone.now() + timezone.timedelta(days=7))
    ...
    def test_user_can_pay_for_an_order_line(self):
        ...
        response = self.client.post(
            url,
            {"payment": payment.id, "payment_price": payment_price.id, "payment_method_id": self.payment_method.id},
            **self.headers,
        )
...
class WebshopOrderTests(OIDCTestCase):
    ...
    def test_user_can_create_orders(self):
        response = self.client.post(self.url, {"product": self.product1.id}, **self.headers)
    ...
    def test_user_can_create_an_order_with_a_size(self):
        ...
        response = self.client.post(self.url, {"product": self.product1.id, "size": product_size_l.id}, **self.headers)
    ...
    def test_user_cannot_create_an_order_with_an_unsupported_size(self):
        ...
        response = self.client.post(
            self.url, {"product": self.product1.id, "size": unsupported_size.id}, **self.headers
        )
    ...
    def test_user_cannot_create_order_with_size_for_products_without_sizes(self):
        ...
        response = self.client.post(
            self.url, {"product": self.product1.id, "size": self.product_size_l.id}, **self.headers
        )
    ...
    def test_user_cannot_create_order_with_product_with_not_enough_stock_of_selected_size(self):
        ...
        response = self.client.post(
            self.url, {"product": self.product1.id, "size": self.product_size_m.id}, **self.headers
        )

        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            response.json().get("size"), [f"Det er ikke flere varer igjen av dette produktet in denne størrelsen"]
        )
