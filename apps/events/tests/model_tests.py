...
class AttendanceEventModelTest(TestCase):
    def setUp(self):
        ...
        self.allowed_username = G(
            AllowedUsername, username="ola123ntnu", expiration_date=self.now + datetime.timedelta(weeks=1)
        )
    ...
    def testRestrictedEvents(self):
        ...
        self.assertTrue(
            unrestricted_event.can_display(allowed_user), "Any user should be able to see unrestricted events"
        )
        self.assertTrue(
            unrestricted_event.can_display(denied_user), "Any user should be able to see unrestricted events"
        )
