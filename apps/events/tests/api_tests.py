...
from .utils import attend_user_to_event, generate_event, generate_payment, generate_user, pay_for_event
...
class CreateAttendeeTestCase(OIDCTestCase):
    ...
    def test_user_cannot_attend_a_non_attendance_event(self, _):
        ...
        response = self.client.post(
            self.url, {"event": event_without_attendance.id, **self.recaptcha_arg}, **self.headers
        )
    ...
    def test_guest_user_can_attend_event_with_guest_attendance(self, _):
        ...
        response = self.client.post(self.url, {"event": self.event.id, **self.recaptcha_arg}, **self.headers)
    ...
    def test_guest_user_cannot_attend_other_users_to_events(self, _):
        ...
        response = self.client.post(
            self.url, {"event": self.event.id, "user": other_user.id, **self.recaptcha_arg}, **self.headers
        )
    ...
    def test_user_can_set_show_as_attending_event_on_registration(self, _):
        ...
        response = self.client.post(
            self.url, {"event": self.event.id, "show_as_attending_event": True, **self.recaptcha_arg}, **self.headers
        )
    ...
    def test_user_can_change_show_as_attending_event_after_registering(self, _):
        ...
        patch_response = self.client.patch(self.id_url(attendee_id), {"show_as_attending_event": True}, **self.headers)
    ...
    def test_user_cannot_unattend_after_event_has_started(self):
        ...
        self.assertEqual(
            response.json().get("message"), "Du kan ikke melde deg av et arrangement som allerde har startet."
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertTrue(self.event.attendance_event.is_attendee(self.user))
        self.assertEqual(
    ...
    def test_payment_is_generated_on_attendance(self, _):
        ...
        attend_response = self.client.post(self.url, {"event": self.event.id, **self.recaptcha_arg}, **self.headers)
        ...
        attendee_response = self.client.get(self.id_url(attendee_id), {"event": self.event.id}, **self.headers)
...
class AttendAPITestCase(OIDCTestCase):
    ...
    def test_attend_with_username(self):
        response = self.client.post(
            self.url, {"event": self.event.id, "username": self.attendee1.user.username}, **self.headers
        )
    ...
    def test_attend_with_rfid(self):
        response = self.client.post(
            self.url, {"event": self.event.id, "rfid": self.attendee1.user.rfid}, **self.headers
        )
    ...
    def test_unknown_rfid(self):
        response = self.client.post(self.url, {"event": self.event.id, "rfid": "fake_rfid"}, **self.headers)
    ...
    def test_saves_rfid(self):
        ...
        response = self.client.post(
            self.url,
            {"event": self.event.id, "username": self.attendee1.user.username, "rfid": "new_rfid"},
            **self.headers,
        )
    ...
    def test_tries_to_save_rfid_with_unknown_username(self):
        response = self.client.post(
            self.url,
            {"event": self.event.id, "username": self.attendee1.user.username + "fake", "rfid": "new_rfid"},
            **self.headers,
        )
    ...
    def test_user_is_on_waitlist_username(self):
        ...
        response = self.client.post(
            self.url, {"event": self.event.id, "username": self.attendee2.user.username}, **self.headers
        )
    ...
    def test_user_is_on_waitlist_rfid(self):
        ...
        response = self.client.post(
            self.url, {"event": self.event.id, "rfid": self.attendee2.user.rfid}, **self.headers
        )
    ...
    def test_user_is_on_waitlist_approved(self):
        ...
        response = self.client.post(
            self.url, {"event": self.event.id, "rfid": self.attendee2.user.rfid, "approved": True}, **self.headers
        )
    ...
    def test_api_does_not_try_to_get_user_by_rfid_empty_string(self):
        ...
        response = self.client.post(self.url, {"event": self.event.id, "rfid": ""}, **self.headers)
    ...
    def test_save_rfid_gives_useful_error_message_if_rfid_already_exists(self):
        ...
        response = self.client.post(
            self.url,
            json.dumps({"event": self.event.id, "username": self.attendee1.user.username, "rfid": rfid}),
            content_type="application/json",
            **self.headers,
        )
    ...
    def test_not_authenticated_user_registering_attendance(self):
        response = self.client.post(
            self.url, {"event": self.event.id, "username": self.attendee1.user.username}, headers={"format": "json"}
        )
