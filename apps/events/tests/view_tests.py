...
class EventsTestMixin:
    def setUp(self):
        ...
        self.event_url = reverse("events_details", args=(self.event.id, self.event.slug))
...
class EventsDetailExtras(EventsTestMixin, TestCase):
    def extras_post(self, event_url, extras_id):
        return self.client.post(
            event_url, {"action": "extras", "extras_id": extras_id}, HTTP_X_REQUESTED_WITH="XMLHttpRequest"
        )
    ...
    def test_extras_on_non_attendance_event(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.json()["message"], "Dette er ikke et påmeldingsarrangement.")
    ...
    def test_extras_on_not_attended_event(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.json()["message"], "Du er ikke påmeldt dette arrangementet.")
...
class EventsAttend(EventsTestMixin, TestCase):
    ...
    def test_attend_not_accepted_rules(self):
        form_params = {"g-recaptcha-response": "PASSED"}
        url = reverse("attend_event", args=(self.event.id,))
        G(AllowedUsername, username=self.user.ntnu_username, expiration_date=timezone.now() + timedelta(days=1))
...
class EventsUnattend(EventsTestMixin, TestCase):
    ...
    def test_unattend_deadline_yesterday(self):
        event = G(Event)
        G(AttendanceEvent, event=event, unattend_deadline=timezone.now() - timedelta(days=1))
        attend_user_to_event(event, self.user)
        url = reverse("unattend_event", args=(event.id,))
        ...
        self.assertRedirects(response, event.get_absolute_url())
        self.assertInMessages("Avmeldingsfristen for dette arrangementet har utløpt.", response)
    ...
    def test_unattend_payment_not_refunded(self):
        ...
        self.assertRedirects(response, event.get_absolute_url())
        self.assertInMessages("Du har betalt for arrangementet og må refundere før du kan melde deg av", response)
...
class EventMailParticipates(EventsTestMixin, TestCase):
    ...
    def test_post_as_arrkom_missing_data(self):
        ...
        self.assertEqual(response.context["event"], event)
        self.assertInMessages("Vi klarte ikke å sende mailene dine. Prøv igjen", response)
        self.assertEqual(len(mail.outbox), 0)
    ...
    def test_post_as_arrkom_successfully(self):
        ...
        response = self.client.post(
            url, {"from_email": "1", "to_email": "1", "subject": "Test", "message": "Test message"}
        )
    ...
    def test_post_as_arrkom_invalid_from_email_defaults_to_kontakt(self):
        ...
        response = self.client.post(
            url, {"from_email": "1000", "to_email": "1", "subject": "Test", "message": "Test message"}
        )
    ...
    def test_post_as_arrkom_invalid_to_email(self):
        ...
        response = self.client.post(
            url, {"from_email": "1", "to_email": "1000", "subject": "Test", "message": "Test message"}
        )
