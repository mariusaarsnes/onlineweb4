...
class SetEventMarks(Task):
    ...
    def run():
        ...
        for attendance_event in attendance_events:
            ...
            if message.send:
                EmailMessage(
                    message.subject, str(message), message.committee_mail, [], message.not_attended_mails
                ).send()
            ...
            if message.committee_message:
                EmailMessage(
                    message.subject, message.committee_message, "online@online.ntnu.no", [message.committee_mail]
                ).send()
    ...
    def active_events():
        return AttendanceEvent.objects.filter(
            automatically_set_marks=True, marks_has_been_set=False, event__event_end__lt=timezone.now()
        )
