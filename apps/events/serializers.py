...
class AttendeeRegistrationReadOnlySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Attendee
        fields = ("id", "event", "user", "attended", "timestamp", "show_as_attending_event", "has_paid", "extras")
        read_only = True
...
class AttendeeRegistrationCreateSerializer(serializers.ModelSerializer):
    ...
    user = serializers.PrimaryKeyRelatedField(
        write_only=True, default=serializers.CurrentUserDefault(), queryset=User.objects.all()
    )
    event = serializers.PrimaryKeyRelatedField(write_only=True, queryset=AttendanceEvent.objects.all())
    ...
    class Meta:
        model = Attendee
        fields = ("id", "event", "user", "show_as_attending_event", "recaptcha", "extras")
...
class RuleBundleSerializer(serializers.ModelSerializer):
    class Meta:
        model = RuleBundle
        fields = ("description", "field_of_study_rules", "grade_rules", "user_group_rules", "rule_strings", "id")
