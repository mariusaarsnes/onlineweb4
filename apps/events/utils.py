...
def handle_event_payment(event, user, payment, context):
    ...
    context.update({"user_paid": user_paid, "payment_delay": payment_delay, "payment_relation_id": payment_relation_id})
...
def handle_mail_participants(
    ...
    try:
        ...
        _email_sent = EmailMessage(
            str(subject), str(message), from_email, [from_email], email_addresses, attachments=(_images)
        ).send()
