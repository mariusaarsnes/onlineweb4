...
class Event(models.Model):
    ...
    author = models.ForeignKey(User, related_name="oppretter", null=True, blank=True, on_delete=models.CASCADE)
...
class FieldOfStudyRule(Rule):
    ...
    def satisfied(self, user, registration_start):
        ...
        return {
            "status": False,
            "message": _("Din studieretning er en annen enn de som har tilgang til dette arrangementet."),
            "status_code": 410,
        }
...
class AttendanceEvent(models.Model):
    ...
    event = models.OneToOneField(Event, primary_key=True, related_name="attendance_event", on_delete=models.CASCADE)
    ...
    automatically_set_marks = models.BooleanField(
        _("automatisk prikk"), default=False, help_text=_("Påmeldte som ikke har møtt vil automatisk få prikk")
    )
...
class CompanyEvent(models.Model):
    ...
    company = models.ForeignKey(Company, verbose_name=_("bedrifter"), on_delete=models.CASCADE)
    event = models.ForeignKey(Event, verbose_name=_("arrangement"), related_name="companies", on_delete=models.CASCADE)
...
class GroupRestriction(models.Model):
    event = models.OneToOneField(Event, primary_key=True, related_name="group_restriction", on_delete=models.CASCADE)

    groups = models.ManyToManyField(
        Group, blank=True, help_text=_("Legg til de gruppene som skal ha tilgang til arrangementet")
    )
