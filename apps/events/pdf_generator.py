...
class EventPDF(object):
    ...
    def create_reservees_table_data(self):
        for reservee in self.reservees:
            self.reservee_table_data.append((create_body_text(reservee.name), create_body_text(reservee.note)))
            if reservee.allergies:
                self.allergies_table_data.append(
                    (create_body_text(reservee.allergies), create_body_text(reservee.name))
                )
                if reservee.allergies:
...
def get_table_style(full_spans=None):
    style = [("GRID", (0, 0), (-1, -1), 0.5, colors.grey), ("BOX", (0, 0), (-1, -1), 1, colors.black)]
    if full_spans:
