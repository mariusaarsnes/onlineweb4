...
from apps.posters.forms import AddBongForm, AddOtherForm, AddPosterForm, EditOtherForm, EditPosterForm
...
def index(request):
    ...
    if not has_view_all_perms(request.user):
        context["your_orders"] = [
            ...
            if request.user.has_perm("view_poster_order", x) and request.user in x.ordered_committee.user_set.all()
        ]
    ...
    if request.is_ajax():
        if request.method == "POST":
            ...
            if request.POST.get("assign_to_id") and not str(request.POST.get("assign_to_id")).isnumeric():
                return HttpResponse(
                    status=400, content=json.dumps({"message": "Denne brukerprofilen kunne ikke tilordnes til ordren."})
                )
