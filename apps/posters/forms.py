...
class AddForm(forms.ModelForm):
    ...
    price = forms.DecimalField(
        label="Pris", required=False, widget=forms.NumberInput(attrs={"placeholder": "Pris på event"})
    )
...
class AddPosterForm(AddForm):
    bong = forms.IntegerField(
        ...
        widget=forms.NumberInput(
            attrs={"placeholder": "Antall bonger du vil ha. La feltet stå tomt hvis du ikke ønsker noen."}
        ),
    )
    event = forms.ModelChoiceField(
        label="Event", queryset=Event.objects.order_by("-id").exclude(event_start__lte=timezone.now())
    )

    class Meta:
        model = Poster
        fields = ["ordered_committee", "event", "amount", "bong", "description", "price", "display_from", "comments"]
...
class AddOtherForm(AddForm):
    title = forms.CharField(
        label="Tittel",
        required=True,
        widget=forms.TextInput(attrs={"placeholder": "Tittelen til det du bestiller, vanligvis arrangementnavn"}),
    )

    class Meta:
        model = Poster
        fields = ["ordered_committee", "title", "amount", "price", "description", "display_from", "comments"]
...
class EditPosterForm(AddPosterForm):
    class Meta:
        model = Poster
        fields = ["event", "amount", "bong", "description", "price", "display_from", "comments"]
