...
def feedback(request, applabel, appmodel, object_id, feedback_id):
    ...
    return render(request, "feedback/answer.html", {"questions": questions, "description": description})
...
def feedback_results(request, feedback_relation, token=False):
    ...
    token_url = "%s%sresults/%s" % (http_host, feedback_relation.get_absolute_url(), register_token.token)
...
def _get_fbr_or_404(app_label, app_model, object_id, feedback_id):
    ...
    try:
        ...
        fbr = FeedbackRelation.objects.get(content_type=ct, object_id=object_id, feedback_id=feedback_id)
