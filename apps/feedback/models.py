...
class FeedbackRelation(models.Model):
    ...
    gives_mark = models.BooleanField(
        _("Gir Prikk"), default=True, help_text=_("Gir automatisk prikk til brukere som ikke har svart innen fristen")
    )
    ...
    def get_absolute_url(self):
        ...
        return reverse(
            "feedback",
            args=[self.content_type.app_label, self.content_type.model, self.object_id, self.feedback.feedback_id],
        )
...
class Feedback(models.Model):
    ...
    display_field_of_study = models.BooleanField(
        _("Vis studieoversikt"), default=True, help_text=_("Grafen over studiefelt vil bli vist til bedriften")
    )
...
class FieldOfStudyAnswer(models.Model):
    feedback_relation = models.ForeignKey(
        FeedbackRelation, related_name="field_of_study_answers", on_delete=models.CASCADE
    )
    answer = models.SmallIntegerField(_("Studieretning"), choices=FieldOfStudyType.ALL_CHOICES)
...
class TextQuestion(models.Model):
    feedback = models.ForeignKey(Feedback, related_name="text_questions", on_delete=models.CASCADE)
...
class TextAnswer(models.Model):
    question = models.ForeignKey(TextQuestion, related_name="answer", on_delete=models.CASCADE)

    feedback_relation = models.ForeignKey(FeedbackRelation, related_name="text_answers", on_delete=models.CASCADE)
...
class RatingQuestion(models.Model):
    feedback = models.ForeignKey(Feedback, related_name="rating_questions", on_delete=models.CASCADE)
...
class RatingAnswer(models.Model):
    feedback_relation = models.ForeignKey(FeedbackRelation, related_name="rating_answers", on_delete=models.CASCADE)
    answer = models.SmallIntegerField(_("karakter"), choices=RATING_CHOICES, default=0)

    question = models.ForeignKey(RatingQuestion, related_name="answer", on_delete=models.CASCADE)
...
class MultipleChoiceRelation(models.Model):
    multiple_choice_relation = models.ForeignKey(MultipleChoiceQuestion, on_delete=models.CASCADE)
    order = models.SmallIntegerField(_("Rekkefølge"), default=30)
    display = models.BooleanField(_("Vis til bedrift"), default=True)
    feedback = models.ForeignKey(Feedback, related_name="multiple_choice_questions", on_delete=models.CASCADE)
...
class MultipleChoiceAnswer(models.Model):
    feedback_relation = models.ForeignKey(
        FeedbackRelation, related_name="multiple_choice_answers", on_delete=models.CASCADE
    )
    answer = models.CharField(_("svar"), blank=False, max_length=256)
    question = models.ForeignKey(MultipleChoiceRelation, related_name="answer", on_delete=models.CASCADE)
...
class RegisterToken(models.Model):
    fbr = models.ForeignKey(FeedbackRelation, related_name="Feedback_relation", on_delete=models.CASCADE)
