...
class RatingAnswerForm(AnswerForm):
    answer = forms.ChoiceField(widget=forms.Select(attrs={"class": "rating", "name": "rating"}), choices=RATING_CHOICES)
...
class TextAnswerForm(AnswerForm):
    answer = forms.CharField(widget=forms.Textarea(attrs={"class": "form-control", "type": "text"}))
