...
def _create_profile_context(request):
    ...
    context = {
        ...
        "approvals": [
            ...
            (
                _("avslåtte søknader"),
                MembershipApproval.objects.filter(applicant=request.user, processed=True, approved=False),
                True,
            ),
    ...
    if request.is_ajax():
        if request.method == "POST":
            ...
            if pos.user == request.user:
                ...
                return_status = json.dumps({"message": _("Du prøvde å slette en posisjon som ikke tilhørte deg selv.")})
...
def delete_email(request):
    if request.is_ajax():
        if request.method == "POST":
            ...
            if email.primary:
                return HttpResponse(
                    status=412, content=json.dumps({"message": _("Kan ikke slette primær-epostadresse.")})
                )
    ...
    if request.is_ajax():
        if request.method == "POST":
            ...
            if email.primary:
                return HttpResponse(
                    status=412,
                    content=json.dumps({"message": _("%s er allerede satt som primær-epostaddresse.") % email.email}),
                )
...
def verify_email(request):
    if request.is_ajax():
        if request.method == "POST":
            ...
            if email.verified:
                return HttpResponse(
                    status=412, content=json.dumps({"message": _("%s er allerede verifisert.") % email.email})
                )
