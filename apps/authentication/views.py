    ...
    if request.user.is_authenticated:
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                messages.success(
                    request, _("Registreringen var vellykket. Se tilsendt epost for verifiseringsinstrukser.")
                )
...
def set_password(request, token=None):
    ...
    if request.user.is_authenticated:
        ...
        if rt and rt.is_valid:
            if request.method == "POST":
                ...
                if form.is_valid():
                    ...
                    messages.success(
                        request,
                        _('Passordgjenoppretting gjennomført for "%s". ' + "Du kan nå logge inn.") % user.username,
                    )
        ...
        messages.error(request, "Noe gikk galt med gjenoppretning av passord. Vennligst prøv igjen.")
        return HttpResponseRedirect("/")
