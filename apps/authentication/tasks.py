...
class SynchronizeGroups(Task):
    ...
    def add_users(sync, logger):
        ...
        users_in_source = User.objects.filter(groups__id__in=sync.get("source")).exclude(
            groups__id__in=sync.get("destination")
        )
    ...
    def remove_users(sync, logger):
        ...
        users_in_destination = (
            User.objects.filter(groups__id__in=sync.get("destination")).all().exclude(groups__id__in=sync.get("source"))
        )
