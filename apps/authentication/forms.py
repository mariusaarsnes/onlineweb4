...
class RegisterForm(forms.Form):
    ...
    first_name = forms.CharField(
        label=_("Fornavn"), max_length=50, help_text="Mellomnavn inkluderer du etter fornavnet ditt"
    )
    last_name = forms.CharField(label=_("Etternavn"), max_length=50)
    email = forms.EmailField(
        label=_("Epost"), max_length=50, help_text="Du kan legge til flere epostadresser senere i din profil."
    )
    password = forms.CharField(widget=forms.PasswordInput(), label=_("Passord"))
    repeat_password = forms.CharField(widget=forms.PasswordInput(), label=_("Gjenta passord"))
    address = forms.CharField(
        label=_("Adresse"), max_length=100, required=False, widget=forms.Textarea(attrs={"rows": 3})
    )
    zip_code = forms.CharField(
        label=_("Postnummer"), max_length=4, required=False, help_text="Vi henter by basert på postnummer"
    )
