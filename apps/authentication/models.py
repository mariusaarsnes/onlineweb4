...
class OnlineUser(AbstractUser):
    ...
    field_of_study = models.SmallIntegerField(
        _("studieretning"), choices=FieldOfStudyType.ALL_CHOICES, default=FieldOfStudyType.GUEST
    )
        ...
        if self.ntnu_username:
            ...
            if (
                AllowedUsername.objects.filter(
                    username=self.ntnu_username.lower(), expiration_date__lt=expiration_threshold
                ).count()
                > 0
            ):
                return True
    ...
    def get_image_url(self, size=50):
        default = "%s%s_%s.png" % (settings.BASE_URL, settings.DEFAULT_PROFILE_PICTURE_PREFIX, self.gender)
...
class OnlineGroup(models.Model):
    ...
    group = models.OneToOneField(
        Group, verbose_name="Djangogruppe", primary_key=True, related_name="online_group", on_delete=models.CASCADE
    )
    ...
    image = models.ForeignKey(
        ResponsiveImage, related_name="online_groups", on_delete=models.DO_NOTHING, null=True, blank=True
    )
...
class GroupMember(models.Model):
    ...
    user = models.ForeignKey(
        OnlineUser, verbose_name="Bruker", related_name="group_memberships", on_delete=models.CASCADE, null=False
    )
    group = models.ForeignKey(
        OnlineGroup, verbose_name="Onlinegruppe", related_name="members", on_delete=models.CASCADE, null=False
    )
...
class GroupRole(models.Model):
    memberships = models.ManyToManyField(GroupMember, verbose_name="Medlemskap", related_name="roles")
