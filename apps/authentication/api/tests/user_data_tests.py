...
class EmailTestCase(OIDCTestCase):
    ...
    def test_user_can_create_an_email(self):
        response = self.client.post(self.url, {"email": "test@example.com"}, **self.headers)
    ...
    def test_user_cannot_create_an_email_with_an_existing_address(self):
        ...
        self.assertEqual(
            second_response.json().get("email"), ["Det eksisterer allerede en bruker med denne e-postadressen"]
        )
    ...
    def test_user_can_change_primary_email(self):
        ...
        response = self.client.patch(self.id_url(email.id), {"primary": True}, **self.headers)
    ...
    def test_user_cannot_remove_primary_email_without_selecting_a_new_primary(self):
        ...
        response = self.client.patch(self.id_url(email.id), {"primary": False}, **self.headers)

        self.email.refresh_from_db()
        email.refresh_from_db()

        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.json().get("primary"), ["Kan bare endre primærepost ved å sette en annen som primær"])
    ...
    def test_user_cannot_verify_emails_without_token(self):
        email: Email = G(Email, user=self.user, verfied=False)
        response = self.client.patch(self.id_url(email.id), {"verified": True}, **self.headers)
    ...
    def test_user_cannot_change_the_address_of_an_existing_email(self):
        ...
        self.client.patch(self.id_url(email.id), {"email": other_address}, **self.headers)
...
class PositionsTestCase(OIDCTestCase):
    def setUp(self):
        ...
        self.other_position: Position = G(
            Position, user=self.other_user, period_start=self.period_start, period_end=self.period_end
        )
        self.position_data = {
            "position": "medlem",
    ...
    def test_user_can_only_create_positions_for_themselves(self):
        response = self.client.post(self.url, {**self.position_data, "user": self.other_user.id}, **self.headers)
    ...
    def test_user_cannot_create_positions_with_start_after_end(self):
        ...
        response = self.client.post(
            self.url, {**self.position_data, "period_end": str(date_before_period_start)}, **self.headers
        )

        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            response.json().get("non_field_errors"), ["Vervets starttid kan ikke være etter vervets sluttid"]
        )

    def test_user_cannot_create_positions_with_start_at_same_time_as_end(self):
        response = self.client.post(
            self.url,
            {**self.position_data, "period_end": str(self.period_start), "period_start": str(self.period_start)},
            **self.headers
        )

        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            response.json().get("non_field_errors"), ["Du kan ikke starte og avslutte et verv på samme dag"]
        )
