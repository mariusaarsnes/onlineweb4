...
class OnlineGroupTestCase(OIDCTestCase):
    ...
    def setUp(self):
        ...
        self.group = G(Group, name=self.group_name)
        self.group_data = {"group": self.group.id, "name_short": self.group_name, "name_long": self.group_name_long}
    ...
    def test_cannot_create_online_group_without_django_group(self):
        wrong_group_id = -1
        response = self.client.post(self.url, {**self.group_data, "group": wrong_group_id}, **self.headers)
...
class GroupMemberTestCase(OIDCTestCase):
    ...
    def test_super_user_can_assign_roles_to_member(self):
        ...
        response = self.client.patch(self.id_url(membership.id), {"roles": role_ids}, **self.headers)
    ...
    def test_group_leader_can_assign_roles_to_member(self):
        ...
        response = self.client.patch(self.id_url(membership.id), {"roles": [role_id]}, **self.headers)
