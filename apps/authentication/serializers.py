...
class UserCreateSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        email = Email.objects.create(user=created_user, email=validated_data.get("email").lower(), primary=True)
        created_user.save()

        send_register_verification_email(user=created_user, email_obj=email, request=self.context.get("request", None))

        return created_user

    class Meta:
        model = User
        fields = ("first_name", "last_name", "password", "email", "username", "recaptcha", "id")
...
class GroupMemberCreateSerializer(serializers.ModelSerializer):
    ...
    group = serializers.PrimaryKeyRelatedField(required=True, queryset=OnlineGroup.objects.all())
    roles = serializers.PrimaryKeyRelatedField(required=False, queryset=GroupRole.objects.all(), many=True)
...
class OnlineGroupCreateOrUpdateSerializer(serializers.ModelSerializer):
    ...
    image = serializers.PrimaryKeyRelatedField(required=False, queryset=ResponsiveImage.objects.all())
