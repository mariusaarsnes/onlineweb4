        ...
        if year:
            if month:
                ...
                queryset = queryset.filter(
                    published_date__year=year, published_date__month=month, published_date__lte=timezone.now()
                ).order_by("-published_date")
            else:
                # Filtering only on year
                queryset = queryset.filter(published_date__year=year, published_date__lte=timezone.now()).order_by(
                    "-published_date"
                )
