...
def study_callback(request):
    ...
    try:
        ...
        if not studies_info:
            logger.warning(
                "Dataporten groups do not match groups for informatics", extra={"user": request.user, "groups": groups}
            )
            messages.error(
                request,
    ...
    if studies_informatics:
        messages.success(
            ...
            "kontakt dotkom slik at vi kan rette opp og finne ut hva som gikk galt.".format(study_name, study_year),
        )
    else:
        messages.error(
            request,
