...
class PaymentRelationCreateSerializer(serializers.ModelSerializer):
    ...
    payment = serializers.PrimaryKeyRelatedField(required=True, queryset=Payment.objects.all())
    payment_price = serializers.PrimaryKeyRelatedField(required=True, queryset=PaymentPrice.objects.all())
    ...
    def create(self, validated_data):
        ...
        payment_relation: PaymentRelation = super().create({**validated_data, "status": status.PENDING})

        logger.info(
            f"Set up Stripe for payment:{payment.id}, user:{request.user.id}, " f"price: {payment_price.price} kr"
        )
        try:
...
class PaymentRelationUpdateSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance: PaymentRelation, validated_data):
        ...
        try:
            ...
            if intent.status == "requires_source_action" and intent.next_action.type == "use_stripe_sdk":
                return super().update(instance, {**validated_data, "payment_intent_secret": intent.client_secret})
            # If the payment is successfully confirmed we update the status of the transaction to SUCCEEDED.
            elif intent.status == "succeeded":
                return super().update(instance, {**validated_data, "status": status.SUCCEEDED})
            ...
            logger.error(
                f"An error occurred during confirmation of " f"PyamentRelation: {instance.id} by user: {instance.user}",
                error,
            )
...
class PaymentTransactionReadOnlySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ("id", "amount", "used_stripe", "datetime", "status", "payment_intent_secret", "description", "items")
        read_only = True
...
class PaymentTransactionCreateSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        transaction: PaymentTransaction = super().create({**validated_data, "status": status.PENDING})
...
class PaymentTransactionUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = PaymentTransaction
        fields = ("payment_intent_id", "id", "payment_intent_secret", "amount", "status", "used_stripe", "datetime")
