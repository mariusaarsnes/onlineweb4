...
from apps.events.tests.utils import attend_user_to_event, generate_event, generate_user, pay_for_event
...
class PaymentRelationTestCase(OIDCTestCase):
    def setUp(self):
        ...
        self.mock_card = {"number": "4242424242424242", "exp_month": 12, "exp_year": date_next_year.year, "cvc": "123"}
    ...
    def test_event_payment_fails_without_payment_method_id(self):
        response = self.client.post(
            self.url, {"payment": self.payment.id, "payment_price": self.payment.price().id}, **self.headers
        )
    ...
    def test_user_cannot_pay_for_event_if_they_are_not_attending(self):
        ...
        response = self.client.post(
            self.url,
            {"payment": payment.id, "payment_price": payment.price().id, "payment_method_id": self.payment_method.id},
            **self.headers,
        )
    ...
    def test_user_can_pay_for_event_with_two_payments(self):
        ...
        response = self.client.post(
            self.url,
            {"payment": self.payment.id, "payment_price": second_price.id, "payment_method_id": self.payment_method.id},
            **self.headers,
        )
...
class PaymentTransactionTestCase(OIDCTestCase):
    def setUp(self):
        ...
        self.id_url = lambda _id: self.url + str(_id) + "/"
        date_next_year = timezone.now() + timezone.timedelta(days=366)
        self.mock_card = {"number": "4000000000003055", "exp_month": 12, "exp_year": date_next_year.year, "cvc": "123"}
        self.mock_3d_secure_card = {
            "number": "4000000000003220",
            "exp_month": 12,
            "exp_year": date_next_year.year,
    ...
    def test_user_can_create_a_transaction_with_a_regular_card(self):
        response = self.client.post(
            self.url, {"amount": self.amount, "payment_method_id": self.payment_method.id}, **self.headers
        )
    ...
    def test_initiating_a_3d_secure_transaction_results_in_a_pending_transaction(self, _):
        response = self.client.post(
            self.url, {"amount": self.amount, "payment_method_id": self.secure_payment_method.id}, **self.headers
        )
    ...
    def test_user_cannot_create_a_transaction_with_wrong_price(self):
        ...
        response = self.client.post(
            self.url, {"amount": wrong_amount, "payment_method_id": self.payment_method.id}, **self.headers
        )
    ...
    def test_event_payment_fails_with_fake_payment_method_id(self):
        ...
        response = self.client.post(
            self.url, {"amount": self.amount, "payment_method_id": fake_payment_method_id}, **self.headers
        )
