    ...
    def destroy(self, request, *args, **kwargs):
        ...
        try:
            ...
            if payment_relation.status in [payment_status.SUCCEEDED, payment_status.DONE]:
                ...
                return Response(
                    {"message": _("Denne betalingen har allerede blitt refundert.")}, status.HTTP_400_BAD_REQUEST
                )
            ...
            logger.error(
                f"An error occurred during refund of payment: {payment_relation} " f"to stripe for user: {user}", error
            )
            return Response(
