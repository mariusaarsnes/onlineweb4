...
class SlackInviteTest(TestCase):
    ...
    def test_invite_okIsNotTrue_raisesException(self, post_mock):
        post_mock.side_effect = [MagicMock(status_code=200, json=lambda: {"ok": False, "error": "Mocked!"})]
    ...
    def test_invite_invalidAuth_logsError(self, post_mock, log_mock):
        post_mock.side_effect = [MagicMock(status_code=200, json=lambda: {"ok": False, "error": "invalid_auth"})]
