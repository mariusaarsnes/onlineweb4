...
def create_g_suite_account(ow4_user):
    ...
    try:
        ...
        logger.error(
            "HttpError while requesting G Suite Account creation: {}".format(err.content), extra={"error": err}
        )
        if err.resp.status == 409:
            logger.error(
                'G Suite account creation: User "{}@online.ntnu.no" already exists.'.format(ow4_user.online_mail)
            )
        raise err
...
def reset_password_g_suite_account(ow4_user):
    ...
    resp = (
        directory.users()
        .update(body={"password": password, "changePasswordAtNextLogin": True}, userKey=user_key)
        .execute()
    )
...
def notify_g_suite_user_account(user, password):
    ...
    message = render_to_string(
        "authentication/email/gsuite_account_notification.txt", {"user": user, "password": password}
    )
