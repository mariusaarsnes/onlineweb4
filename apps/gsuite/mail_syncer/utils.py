...
def get_user_key(domain, user):
    ...
    if not domain or not user:
        logger.error(
            "You need to pass a domain and a user when generating user key.", extra={"domain": domain, "group": user}
        )
...
def insert_email_into_g_suite_group(domain, group_name, email, suppress_http_errors=False):
    ...
    logger.info(
        "Inserting '{email}' into G Suite group '{group}'.".format(email=email, group=group_key),
...
def insert_ow4_user_into_g_suite_group(domain, group_name, ow4_user, suppress_http_errors=False):
    ...
    """
    logger.info(
        "Inserting '{user}' into G Suite group '{group}'.".format(user=ow4_user, group=group_name),
    ...
    return insert_email_into_g_suite_group(
        domain, group_name, ow4_user.online_mail, suppress_http_errors=suppress_http_errors
    )
...
def remove_g_suite_user_from_group(domain, group_name, g_suite_user, suppress_http_errors=False):
    ...
    logger.info(
        "Removing '{user}' from G Suite group '{group}'.".format(user=user_key, group=group_key),
    ...
    try:
        ...
        logger.error(
            'HttpError when deleting user "{user}" from G Suite group: {err}'.format(err=err, user=user_key),
            extra={"suppress_http_error": suppress_http_errors},
        )
        if not suppress_http_errors:
            raise err
...
def get_g_suite_groups_for_user(domain, _user, suppress_http_errors=False):
    ...
    logger.debug("Getting G Suite user membership list for '{user}'.".format(user=user_key), extra={"user": user_key})
    ...
    if ow4_count == g_suite_count:
        ...
        if not quiet:
            logger.debug(
                ...
                "Need to trim inactive users from G Suite.".format(g_suite_count=g_suite_count, ow4_count=ow4_count)
            )
    else:
        if not quiet:
            logger.debug(
                "There are more users on OW4 ({ow4_count}) than in G Suite ({g_suite_count}). "
                "Need to update G Suite with new members.".format(g_suite_count=g_suite_count, ow4_count=ow4_count)
            )
    return False
