...
class GSuiteAPITestCase(TestCase):
    ...
    def test_insert_when_insert_disabled(self, mocked_logger):
        ...
        with override_settings(OW4_GSUITE_SYNC=ow4_gsuite_sync):
            ...
            mocked_logger.assert_called_with(
                'Skipping inserting email "{email}" since ENABLE_INSERT is False.'.format(email=email)
            )
    ...
    def test_remove_when_remove_disabled(self, mocked_logger):
        ...
        with override_settings(OW4_GSUITE_SYNC=ow4_gsuite_sync):
            ...
            self.assertIsNone(resp)
            mocked_logger.assert_called_with(
                'Skipping removing user "{user}" since ENABLE_DELETE is False.'.format(user=email)
            )
    ...
    @patch("logging.Logger.error")
    @patch("apps.gsuite.mail_syncer.utils.setup_g_suite_client", autospec=True)
    def test_remove_g_suite_user_from_g_suite_group_g_suite_dict_httperror(self, mocked_g_suite_client, mocked_logger):
        group_name = list(settings.OW4_GSUITE_SYNC.get("GROUPS").keys())[0]
        email = "example@example.org"
        ...
        with override_settings(OW4_GSUITE_SYNC=ow4_gsuite_sync):
            self.assertRaises(
                HttpError,
                lambda: remove_g_suite_user_from_group(self.domain, group_name, email, suppress_http_errors=False),
            )
        mocked_logger.assert_called_with(
            'HttpError when deleting user "{user}" from G Suite group: {err}'.format(err=http_error, user=email),
