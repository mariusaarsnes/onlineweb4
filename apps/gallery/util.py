...
class BaseImageHandler(object):
    ...
    def create_thumbnail(self):
        ...
        if isinstance(self, UploadImageHandler):
            ...
            thumbnail_path = os.path.join(
                django_settings.MEDIA_ROOT, gallery_settings.UNHANDLED_THUMBNAIL_PATH, filename
            )
            thumbnail_path = os.path.abspath(thumbnail_path)

            # Generate the full path to the origin image
            original_path = os.path.join(django_settings.MEDIA_ROOT, gallery_settings.UNHANDLED_IMAGES_PATH, filename)
        ...
        # If this object is a ResponsiveImageHandler instance, create thumbnails for the responsive image
            ...
            thumbnail_dest = os.path.join(
                django_settings.MEDIA_ROOT, gallery_settings.RESPONSIVE_THUMBNAIL_PATH, filename
            )
            ...
            return GalleryStatus(
                False, 'Could not decide what thumbnail to generate, since "self" is %s' % type(self), None
            )
    ...
    def _copy_file(from_path, to_path):
        ...
        try:
            ...
            return GalleryStatus(
                False, "Failed to copy image from %s to %s: %s" % (from_path, to_path, os_error), os_error
            )
...
class ResponsiveImageHandler(BaseImageHandler):
    ...
    def _crop_image(self):
        ...
        destination_path = os.path.join(
            django_settings.MEDIA_ROOT, gallery_settings.RESPONSIVE_IMAGES_PATH, os.path.basename(relative_source_path)
        )
    ...
    def _generate_downsized_versions(self):
        ...
        wide_destination_path = os.path.join(
            django_settings.MEDIA_ROOT, gallery_settings.RESPONSIVE_IMAGES_WIDE_PATH, os.path.basename(source_path)
        )
        lg_destination_path = os.path.join(
            django_settings.MEDIA_ROOT, gallery_settings.RESPONSIVE_IMAGES_LG_PATH, os.path.basename(source_path)
        )
        md_destination_path = os.path.join(
            django_settings.MEDIA_ROOT, gallery_settings.RESPONSIVE_IMAGES_MD_PATH, os.path.basename(source_path)
        )
        sm_destination_path = os.path.join(
            django_settings.MEDIA_ROOT, gallery_settings.RESPONSIVE_IMAGES_SM_PATH, os.path.basename(source_path)
        )
        xs_destination_path = os.path.join(
            django_settings.MEDIA_ROOT, gallery_settings.RESPONSIVE_IMAGES_XS_PATH, os.path.basename(source_path)
        )
        ...
        wide = self._resize_image(
            source_path, wide_destination_path, gallery_settings.MODELS[self._config["preset"]]["sizes"]["lg"]
        )
        lg = self._resize_image(
            source_path, lg_destination_path, gallery_settings.MODELS[self._config["preset"]]["sizes"]["lg"]
        )
        md = self._resize_image(
            source_path, md_destination_path, gallery_settings.MODELS[self._config["preset"]]["sizes"]["md"]
        )
        sm = self._resize_image(
            source_path, sm_destination_path, gallery_settings.MODELS[self._config["preset"]]["sizes"]["sm"]
        )
        xs = self._resize_image(
            source_path, xs_destination_path, gallery_settings.MODELS[self._config["preset"]]["sizes"]["xs"]
        )
...
def check_crop_bounds(crop_anchor, crop_size, min_size, max_size):
    ...
    if not valid:
        return GalleryStatus(
            False, "The provided crop attributes provided out of bounds values", (crop_anchor, crop_size)
        )
...
def get_absolute_path_to_original(image):
    ...
    if isinstance(image, UnhandledImage):
        path = os.path.abspath(os.path.join(django_settings.MEDIA_ROOT, image.image.name))
    elif isinstance(image, ResponsiveImage):
        path = os.path.abspath(os.path.join(django_settings.MEDIA_ROOT, image.image_original.name))
