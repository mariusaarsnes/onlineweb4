...
def _create_request_dictionary():
    ...
    dictionary = {"unhandled_images": UnhandledImage.objects.all(), "responsive_images": ResponsiveImage.objects.all()}
    ...
    if request.is_ajax():
        if request.method == "GET":
            ...
            for image in UnhandledImage.objects.all():
                images.append({"id": image.id, "thumbnail": image.thumbnail.url, "image": image.image.url})
...
class CropView(PermissionRequiredMixin, View):
    ...
    def post(self, *args, **kwargs):
        ...
        self.log.info(
            "%s cropped and saved ResponsiveImage %d (%s)" % (self.request.user, resp_image.id, config.get("name"))
        )
...
def crop(request):
    ...
    if request.is_ajax():
        if request.method == "POST":
            ...
            # Log who performed the crop operation
            log.info("%s cropped and saved ResponsiveImage %d (%s)" % (request.user, resp_image.id, config.get("name")))
...
def search(request):
    ...
    matches = ResponsiveImage.objects.filter(
        Q(name__icontains=query) | Q(description__icontains=query) | Q(tags__name__in=query.split(" "))
    ).distinct()[:15]
    ...
    def get_queryset(self):
        ...
        if year:
            if month:
                ...
                queryset = queryset.filter(timestamp__year=year, timestamp__month=month).order_by("-timestamp")
            else:
